...
//NODE SPECIFIC DATA
extern	uint32_t my_id; 	//this node's ID
extern	uint32_t my_sequence_number; 	//this node's sequence number
extern	uint32_t rreq_id; 				//id for RREQ
...
extern	uint8_t rx_data[256];			//Buffer for holding received data
...
extern	struct noroute_table_entry noroute_table[NOROUTE_QUEUE_MAX_ENTRIES];						//TODO, this needs to be a linkedlist
extern	uint8_t noroute_table_entries;

#define RREQ_TABLE_MAX_ENTRIES 10

extern	uint32_t rreq_table[RREQ_TABLE_MAX_ENTRIES];
extern	uint8_t rreq_pointer;
